...
(
 ...
 uniform highp float points[256];
...
- (id)initWithFragmentShaderFromString:(NSString *)fragmentShaderString;
{
    ...
    pointsCountUniform = [filterProgram uniformIndex:@"pointsCount"];
    ...
    max = 64;
    points = malloc(max * 2 * sizeof(GLfloat));
    pointsCount = 0;
...
- (void)addPoint:(CGPoint)point
{
    ...
    {
        max *= 2;
        points = realloc(points, max * 2 * sizeof(GLfloat));
    }
    
    points[pointsCount++] = point.x;
    points[pointsCount++] = point.y;
    
    [self setFloatArray:points length:pointsCount forUniform:@"points"];
    [self setInteger:pointsCount forUniformName:@"pointsCount"];
}
